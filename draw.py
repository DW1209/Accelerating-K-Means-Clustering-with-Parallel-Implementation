...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--filename', type=str, help='filename before k-means clustering', required=True)
    ...
    # before
    df = pd.read_csv(args.filename, delim_whitespace=True, header=None)
    df.columns = ['x', 'y']
    sns.scatterplot(ax=ax[0], x=df['x'], y=df['y'])
    ax[0].set_title('Before clustering')

    # after
    df = pd.read_csv(f'{args.filename}.out', delim_whitespace=True, header=None)
